 ...
 * SPDX-FileCopyrightText: Lunaris Project
...
class TrickyStore : SettingsPreferenceFragment() {
                ...
                onSuccess = { xml ->
                    ...
                    try {
                        ...
                        if (existingXml != null && existingXml.trim() == xml.trim()) {
                            if (!silent) toast(getString(R.string.ts_fetch_keybox_same_file), isLong = true)
                            return@fold
                        }
                        ...
                        if (leafExpiry != null && leafExpiry.time < now) {
                            if (!silent) toast(getString(
                                R.string.ts_fetch_keybox_failed, "Cert already expired"), isLong = true)
                        ...
                        if (fetchedInvalid) {
                            toast(getString(R.string.ts_fetch_keybox_no_valid_found), isLong = true)
                            markNoValidKeyboxFound()
                            return@fold
                        }
                        ...
                        saveLastFetchedTimestamp()
                        killGms()
                        val successMsg = getString(
                            if (silent) R.string.ts_fetch_keybox_auto_replaced
                            else R.string.ts_fetch_keybox_success
                        )
                        toast("$successMsg\n${getString(R.string.ts_crafted_by)}", isLong = true)
                        currentRevocationStatus = RevocationStatus.UNKNOWN
                        currentRevocationReason = ""
                        refreshStatus()
                        checkKeyboxRevocation()
                    } catch (e: Exception) {
                        if (!silent) toast(getString(R.string.ts_fetch_keybox_failed, e.message ?: ""), isLong = true)
                    }
                },
                onFailure = { e ->
                    if (!silent) toast(getString(R.string.ts_fetch_keybox_failed, e.message ?: ""), isLong = true)
    ...
    private fun toast(msg: String, isLong: Boolean = false) {
        val t = Toast.makeText(requireContext(), msg, if (isLong) Toast.LENGTH_LONG else Toast.LENGTH_SHORT)
        t.show()
        if (isLong) {
            android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                try { t.show() } catch (_: Exception) {}
            }, 2000L)
        }
    }
